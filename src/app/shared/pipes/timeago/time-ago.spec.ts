...
describe('TimeAgoPipe', () => {
  ...
  it('formats seconds ago', () => {
    const result = pipe.transform(new Date(now.getTime() - 30 * SEC));
    expect(result).toBe('30 seconds ago');
  });

  it('formats 1 minute ago', () => {
    const result = pipe.transform(new Date(now.getTime() - MIN));
    expect(result).toBe('1 minute ago');
  });

  it('formats minutes ago', () => {
    const result = pipe.transform(new Date(now.getTime() - 45 * MIN));
    expect(result).toBe('45 minutes ago');
  });

  it('formats hours ago', () => {
    const result = pipe.transform(new Date(now.getTime() - 3 * HOUR));
    expect(result).toBe('3 hours ago');
  });

  it('formats days ago', () => {
    const result = pipe.transform(new Date(now.getTime() - 5 * DAY));
    expect(result).toBe('5 days ago');
  });

  it('formats weeks ago', () => {
    const result = pipe.transform(new Date(now.getTime() - 2 * WEEK));
    expect(result).toBe('2 weeks ago');
  });

  it('formats months ago', () => {
    const result = pipe.transform(new Date(now.getTime() - 3 * MONTH));
    expect(result).toBe('3 months ago');
  });

  it('formats years ago', () => {
    const result = pipe.transform(new Date(now.getTime() - 2 * YEAR));
  ...
  it('formats seconds in the future', () => {
    const result = pipe.transform(new Date(now.getTime() + 10 * SEC));
    expect(result).toBe('in 10 seconds');
  });

  it('formats hours in the future', () => {
    const result = pipe.transform(new Date(now.getTime() + 2 * HOUR));
  ...
  it('re-formats when the language changes', () => {
    ...
    TestBed.runInInjectionContext(() => langChanges$.next('fr'));
    TestBed.tick();
  ...
  it('re-formats when the language changes', () => {
    ...
    TestBed.runInInjectionContext(() => langChanges$.next('ar'));
    TestBed.tick();
