...
@Component({
  ...
  template: `
    <hlm-popover sideOffset="5" [state]="_popoverState()" (stateChanged)="_popoverState.set($event)" (closed)="_onClose()">
      <button [id]="buttonId()" type="button" [class]="_computedClass()" [disabled]="_mutableDisabled()" hlmPopoverTrigger>
        <span class="truncate">
